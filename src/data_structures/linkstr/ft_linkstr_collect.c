...
/*   Updated: 2023/01/11 10:28:49 by emcnab           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 *
 * @param linkstr (t_s_linkstr *): the linked string containing the strings.
 *
 * @return (char *): result of concatenating all the strings in [linkstr].
 */
char	*ft_linkstr_collect(t_s_linkstr *linkstr)
{
	char	*collect;
	t_s_list	*cur;
	size_t	i;
	...
	if (!linkstr)
		return (NULL);
	collect = malloc((ft_s_linkstr_size(linkstr) + 1) * sizeof(*collect));
